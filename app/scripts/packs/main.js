...
L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiYWRlYnJ1aW5lIiwiYSI6ImNqYWNyczh1YjBjdjAyd28ybnVrN2FieDcifQ.4MiOQFZ5P1NJ9yyPiiGSWA', {
    attribution: 'Map data &copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a> contributors, <a href="https://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery © <a href="https://www.mapbox.com/">Mapbox</a>',
    maxZoom: 18,
    id: 'mapbox.streets',
    accessToken: 'pk.eyJ1IjoiYWRlYnJ1aW5lIiwiYSI6ImNqYWNyczh1YjBjdjAyd28ybnVrN2FieDcifQ.4MiOQFZ5P1NJ9yyPiiGSWA'
}).addTo(mymap);
...
(function makeMarkers() {
  for (var i in dormList) {
    var marker = L.marker([dormList[i].lat, dormList[i].long]).setIcon(grayIcon)
    marker.addTo(mymap);
...
allFilters.ac = dormList
  ...
  allFilters.AC = dormList.filter(function(dorm){
      return dorm.has_ac == true;
    }).map(function(dorm){
      return dorm.name;
  });

  allFilters.dining = dormList.filter(function(dorm){
      return (dorm.dining == true);
    }).map(function(dorm){
      return dorm.name;
  });
// dorm size
allFilters.small = dormList.filter(function(dorm){
    return (dorm.size <= 100);
  }).map(function(dorm){
    return dorm.name;
});

allFilters.medium = dormList.filter(function(dorm){
    return (dorm.size > 100 && dorm.size <= 200);
  }).map(function(dorm){
    return dorm.name;
});

allFilters.large = dormList.filter(function(dorm){
